...
from settings import FILL_VAL, TOKENS_WEIGHT, args
...
class EWC(Regularizer):
    ...
    def task_end_do(self):
        updater = Omega_update(self.model.parameters(), lr=0.0001, momentum=0.9)
        compute_importance(
            self.model, self.parallel_model, updater, self.dataloaders, loss_type="ewc"
        )
        accumulate_reg_params(self.model)
        self.save_reg_params()
...
class Omega_update(SGD):
    ...
    def __init__(
        self, params, lr=0.001, momentum=0, dampening=0, weight_decay=0, nesterov=False
    ):

        super(Omega_update, self).__init__(
...
def compute_importance(model, parallel_model, updater, dataloaders, loss_type="l2"):
    ...
    if loss_type == "l2":
        loss_fct = DataParallelCriterion(
            torch.nn.MSELoss(reduction="mean"), args.device_ids
        )
    elif loss_type == "l1":
        loss_fct = DataParallelCriterion(
            torch.nn.L1Loss(reduction="mean"), args.device_ids
        )
    elif loss_type == "ewc":
        CELoss = CrossEntropyLoss(
            ignore_index=FILL_VAL, reduction="mean", weight=TOKENS_WEIGHT
        )
        loss_fct = DataParallelCriterion(CELoss, args.device_ids)

    # Iterate over data.
    for dataloader in dataloaders:
        for _, cq, len_cq, cqa, len_cqa, Y, _, _, _ in dataloader:
            # get the inputs
            ...
            if loss_type != "ewc":
                ...
                logits = [
                    logit[range(len(logit)), len_cq[i] - 1, :]
                    for i, logit in enumerate(logits)
                ]
                # logits = [softmax(logit, dim=-1) for logit in logits]
                target_zeros = [
                    torch.zeros(logit.size()).to(args.device_ids[i])
                    for i, logit in enumerate(logits)
                ]
                ...
                targets = get_losses(parallel_model, cqa, Y, None, None, loss_fct)
...
class Weight_Regularized_SGD(SGD):
    ...
    def __init__(
        ...
        super(Weight_Regularized_SGD, self).__init__(
            params, lr, momentum, dampening, weight_decay, nesterov
        )
